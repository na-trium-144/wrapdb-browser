...
export default function PackageDetailPage() {
  ...
  const renderSection = (title: string, content: React.ReactNode) => (
    <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
      <h2 className="text-xl font-semibold text-gray-300 border-b border-gray-700 pb-2 mb-4">{title}</h2>
      {content}
    </div>
  );

  const renderNameList = (names: string[]) => {
    if (names.length === 0) return <p className="text-gray-400">None</p>;
    return (
      <div className="flex flex-wrap gap-2">
        {names.map(name => (
          <span key={name} className="px-3 py-1 bg-gray-700 text-gray-200 text-sm rounded-full">
            {name}
          </span>
        ))}
      </div>
    ...
    <div className="bg-gray-950 text-gray-100 min-h-screen pt-10 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-4xl mx-auto">
        <header className="mb-8">
          <Link to="/" className="text-blue-400 hover:text-blue-500 transition-colors">&larr; Back to search</Link>
          <h1 className="text-5xl font-bold mt-4">{pkg.name}</h1>
          <p className="text-lg text-gray-400 mt-2">{pkg.description}</p>
        </header>
        ...
        <main className="space-y-6">
          {renderSection("Available Versions", (
            <div className="flex flex-wrap gap-2">
              {pkg.versions.length > 0 ? (
                pkg.versions.map(v => (
                  <span key={v} className={`px-3 py-1 text-sm rounded-full ${v === pkg.latest_version ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200'}`}>
                    {v}
                  </span>
                ))
              ) : (
          ...
          {renderSection("Usage (meson.build)", (
            <pre className="bg-gray-800 p-4 rounded-md text-sm text-gray-200 overflow-x-auto">
              <code>{pkg.usage}</code>
            </pre>
          ))}
        </main>
